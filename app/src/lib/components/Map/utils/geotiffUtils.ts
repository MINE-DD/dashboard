...
function transformBoundsToWgs84(bounds: [number, number, number, number]): [number, number, number, number] {
  ...
  // Check if these are global Web Mercator bounds
  // (-20037508, -10018754, 20037508, 10018754) which is the entire world
  const isGlobalMercator =
    Math.abs(bounds[0] + 20037508) < 100 &&
    Math.abs(bounds[1] + 10018754) < 100 &&
    Math.abs(bounds[2] - 20037508) < 100 &&
    Math.abs(bounds[3] - 10018754) < 100;

  if (isGlobalMercator) {
    console.log('GeoTIFF: Detected global Web Mercator bounds, using Africa-specific bounding box');
    return getAfricaBoundingBox();
  }

  // Regular transformation for non-global bounds
...
export async function renderGeoTIFFOnMap(
  ...
  try {
    ...
    // Create coordinates in the format MapLibre expects - flipping them properly to fix upside-down issue
    const coordinates: [
      [number, number],
      [number, number],
      [number, number],
    ...
    // Fill the ImageData - with flipped Y to fix upside down issue
    for (let y = 0; y < height; y++) {
      for (let x = 0; x < width; x++) {
        // Flip Y coordinate (read from bottom to top instead of top to bottom)
        const srcIdx = (height - 1 - y) * width + x;
        const destIdx = y * width + x;

        const value = band[srcIdx];
